...
public class ClanApplyRunnable extends BukkitRunnable {
	...
	public void run() {
		...
		for (Clan clan : topClans) {
			...
			ConfigurationSection configSection = config.getConfigurationSection("clan").getConfigurationSection(String.valueOf(counter));
			
			List<Location> signLocationList = (List<Location>) configSection.getList("sign_location");
			List<Location> bannerLocationlist = (List<Location>) configSection.getList("banner_location");
			if (signLocationList != null)
				for (Location signLocation : signLocationList)
					setClanSign(clan, signLocation);
			
			if (bannerLocationlist != null)
				for (Location bannerLocation : bannerLocationlist)
					setClanBanner(clan, bannerLocation);
			
			counter++;
		}
		new PlayerScoreCalculateRunnable().runTaskAsynchronously(CLHeroesPlugin.getInstance());
	}
